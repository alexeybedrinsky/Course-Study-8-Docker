...
class HabitAPITestCase(TestCase):
    ...
    def test_update_habit(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Habit.objects.get(id=habit.id).action, 'Вечерняя пробежка')
...
class PermissionsTestCase(TestCase):
    def setUp(self):
        ...
        self.habit = Habit.objects.create(user=self.user, action='Test Habit', place='Home', time='12:00:00', duration=60)
...
class SerializerTestCase(TestCase):
    ...
    def test_public_habit_serializer(self):
        ...
        self.assertEqual(set(serializer.data.keys()), set(['id', 'place', 'time', 'action', 'duration']))
