...
class HabitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data.get('is_pleasant'):
            ...
            raise serializers.ValidationError("Привычка должна иметь связанную привычку или вознаграждение.")

        if data.get('duration'):
            if data['duration'] > 120:
                raise serializers.ValidationError("Длительность привычки не может превышать 120 секунд.")
            if data['duration'] < 1:
                raise serializers.ValidationError("Длительность привычки должна быть положительным числом.")

        if data.get('frequency'):
            if data['frequency'] > 7:
                raise serializers.ValidationError("Нельзя выполнять привычку реже, чем раз в 7 дней.")
            if data['frequency'] < 1:
                raise serializers.ValidationError("Частота выполнения должна быть положительным числом.")
